...
function MakeAsync.new(clear_func, append_func)
  local self = setmetatable({}, MakeAsync)
...
function MakeAsync:jump_to_bottom()
...
function MakeAsync:set_keymap(finalize_func)
...
function MakeAsync:get_phase_title(cmd)
...
function MakeAsync:execute(command_provider)
...
function MakeAsync.setup()
  local qf = MakeAsync.new(qf_clear, qf_append)
  ...
  vim.api.nvim_set_keymap('n', '<leader>mm', '', { noremap = true, callback = make, desc = "Run 'makeprg' asynchronously and populate quickfix" })
  vim.api.nvim_create_user_command('X',
    function(a) qf:execute(function() return a.args end) end,
